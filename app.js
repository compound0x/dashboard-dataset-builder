...
const api = {
  ...
  async exportDataset(format) {
    try {
      ...
      triggerDownload(new Blob([payload], { type: "application/octet-stream" }), `dataset.${format}`);
...
function getPreviewPayload() {
  return {
    ...
    draftRecord: appState.draftRecord,
  };
}

function syncPreviewFromState(force = false) {
  if (previewDirty && !force) return;
  previewDraftText = JSON.stringify(getPreviewPayload(), null, 2);
  previewDirty = false;
}

function hydrateDraftBuilders() {
  const metaEntries = Object.entries(appState.draftRecord.meta || {});
  const assetEntry = metaEntries.find(([key]) => key === "asset");
  const timestampEntry = metaEntries.find(([key]) => key === "timestamp");

  appState.draftRecord.meta.asset = assetEntry?.[1] || "";
  appState.draftRecord.meta.timestamp = timestampEntry?.[1] || "";
  customMetaRows = metaEntries
    .filter(([key]) => key !== "asset" && key !== "timestamp")
    .map(([key, value]) => ({ key, value }));

  featureRows = Object.entries(appState.draftRecord.features || {}).map(([name, value]) => ({
    name,
    type: typeof value === "number" && Number.isInteger(value) ? "int" : typeof value === "number" ? "float" : typeof value === "boolean" ? "boolean" : "string",
    value: String(value),
  }));
  if (featureRows.length === 0) featureRows = [{ name: "", type: "float", value: "" }];

  labelRows = Object.entries(appState.draftRecord.labels || {}).map(([name, value]) => ({
    name,
    type: typeof value === "number" && Number.isInteger(value) ? "int" : typeof value === "number" ? "float" : typeof value === "boolean" ? "boolean" : "string",
    value: String(value),
  }));
  if (labelRows.length === 0) labelRows = [{ name: "", type: "string", value: "" }];
...
function syncMetaRowsToDraft() {
  ...
  customMetaRows.forEach((row) => {
    if (!row.key || Object.hasOwn(meta, row.key)) return;
...
function populateFromParsedDataset(data) {
  if (Array.isArray(data)) {
    appState.records = data;
  } else {
    appState.datasetInfo = {
      ...DEFAULT_STATE.datasetInfo,
      ...(data.datasetInfo || {}),
    };
    appState.records = Array.isArray(data.records) ? data.records : [];
    appState.draftRecord = {
      ...DEFAULT_STATE.draftRecord,
      ...(data.draftRecord || {}),
      meta: {
        ...DEFAULT_STATE.draftRecord.meta,
        ...(data.draftRecord?.meta || {}),
      },
      features: data.draftRecord?.features || {},
      labels: data.draftRecord?.labels || {},
      tags: Array.isArray(data.draftRecord?.tags) ? data.draftRecord.tags : [],
    };
  }

  hydrateDraftBuilders();
  syncPreviewFromState(true);
  render();
}
...
function importDatasetFile(event) {
  ...
  reader.onload = () => {
    try {
      ...
      populateFromParsedDataset(parsed);
      showToast("Dataset imported", "success");
    } catch {
...
function generateSampleDataset() {
  const now = new Date();
  appState.datasetInfo = {
    name: "quickstart-demo-dataset",
    version: "1.0.0",
    description: "A starter dataset generated automatically to help new users understand structure.",
  };
  appState.records = [0, 1, 2].map((index) => ({
    id: crypto.randomUUID(),
    meta: {
      asset: "image",
    ...
    features: {
      brightness: Number((0.52 + index * 0.11).toFixed(2)),
      width: 1024 - index * 128,
      has_face: index % 2 === 0,
    },
    labels: {
      class_name: index === 0 ? "cat" : index === 1 ? "dog" : "bird",
      confidence: Number((0.92 - index * 0.1).toFixed(2)),
    },
    tags: ["sample", "quickstart", `record-${index + 1}`],
  }));
  appState.draftRecord = structuredClone(DEFAULT_STATE.draftRecord);
  hydrateDraftBuilders();
  syncPreviewFromState(true);
  render();
  showToast("Sample dataset generated", "success");
}

function handlePreviewInput(value) {
  previewDraftText = value;
  previewDirty = true;
}

function applyPreviewJSON() {
  try {
    const parsed = JSON.parse(previewDraftText);
    if (!parsed || typeof parsed !== "object") {
      showToast("Preview JSON must be an object", "error");
      return;
    }
    populateFromParsedDataset(parsed);
    previewDirty = false;
    showToast("Preview JSON applied", "success");
  } catch {
    showToast("Preview contains invalid JSON", "error");
  }
}
  ...
  app.innerHTML = `
    <section>
      <div class="panel">
        ...
        <p class="muted">Build records manually, import an external JSON file, or generate a sample dataset to learn by example.</p>
        <div class="button-row">
          <button onclick="generateSampleDataset()">Generate Sample Dataset</button>
          <label class="file-upload">Import Dataset JSON
            <input type="file" accept="application/json" onchange="importDatasetFile(event)" />
          </label>
        </div>
      </div>

      <div class="panel">
        <h2>Dataset Header</h2>
    ...
    <aside>
      ...
      <div class="panel">
        <h2>Editable JSON Preview</h2>
        <p class="muted">Edit and apply JSON to quickly modify dataset info, records, and draft state.</p>
        <textarea class="json-preview" oninput="handlePreviewInput(this.value)">${previewDraftText}</textarea>
        <div class="button-row">
          <button class="primary" onclick="applyPreviewJSON()">Apply JSON Edits</button>
          <button onclick="syncPreviewFromState(true); render();">Reset Preview</button>
        </div>
      </div>
    </aside>
  `;
